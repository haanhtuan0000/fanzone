...
export class VariableResolverService {
  ...
  async buildMatchContext(
    ...
    const ctx: MatchContext = {
      ...
      home_striker: lineup?.home?.strikers?.[0] ?? `${teams.home} striker`,
      away_striker: lineup?.away?.strikers?.[0] ?? `${teams.away} striker`,
      home_midfielder: lineup?.home?.midfielders?.[0] ?? `${teams.home} midfielder`,
      away_midfielder: lineup?.away?.midfielders?.[0] ?? `${teams.away} midfielder`,
      home_keeper: lineup?.home?.goalkeeper ?? `${teams.home} goalkeeper`,
      away_keeper: lineup?.away?.goalkeeper ?? `${teams.away} goalkeeper`,
      risky_player_home: lineup?.home?.midfielders?.[1] ?? 'Other player',
      risky_player_away: lineup?.away?.midfielders?.[1] ?? 'Other player',
      home_sub_striker: lineup?.home?.strikers?.[1] ?? 'Substitute',
      away_sub_striker: lineup?.away?.strikers?.[1] ?? 'Substitute',
      sub_midfielder: lineup?.home?.midfielders?.[2] ?? 'Substitute',
